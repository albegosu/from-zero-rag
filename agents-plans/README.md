...
If you want this material **out of the git tree** entirely, stop tracking the folder (e.g. `git rm -r --cached agents-plans` then add `/agents-plans/` to `.gitignore`) and keep copies locally—only do that if the whole team agrees, since history and clones will no longer include these files.
